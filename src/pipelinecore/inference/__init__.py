...
from .schema import Analysis, InferenceCmd, InferenceCmdItem, InferenceEnum, Task  # MODEL_MAPPING_SERIES_DICT,
...
def generate_output_files(input_paths: list[str], task_name: str, base_output_path: str) -> list[str]:
    ...
    match task_name:
        case InferenceEnum.Aneurysm:
            output_files.append(os.path.join(base_output_path, "Pred_Aneurysm.nii.gz"))
            output_files.append(os.path.join(base_output_path, "Prob_Aneurysm.nii.gz"))
            output_files.append(os.path.join(base_output_path, "Pred_Aneurysm_Vessel.nii.gz"))
            output_files.append(os.path.join(base_output_path, "Pred_Aneurysm.json"))
            output_files.append(os.path.join(base_output_path, "Pred_Aneurysm_Vessel16.nii.gz"))
            ...
            if base_name1.startswith("SWAN"):
                ...
                output_files.append(
                    os.path.join(
                        base_output_path,
                        (
                            f"synthseg_{base_name1}_original_{task_name}_from_"
                ...
                output_files.append(
                    os.path.join(
                        base_output_path,
                        (
                            f"synthseg_{base_name2}_original_{task_name}_from_"
                            f"synthseg_{base_name1}_original_{task_name}.nii.gz"
                        ),
                    )
                )
            output_files.append(os.path.join(base_output_path, "Pred_CMB.nii.gz"))
            output_files.append(os.path.join(base_output_path, "Pred_CMB.json"))
        ...
        case InferenceEnum.Infarct:
            output_files.append(os.path.join(base_output_path, "Pred_Infarct.nii.gz"))
            output_files.append(os.path.join(base_output_path, "Pred_Infarct_ADCth.nii.gz"))
            output_files.append(os.path.join(base_output_path, "Pred_Infarct_synthseg.nii.gz"))
            output_files.append(os.path.join(base_output_path, "Pred_Infarct.json"))
        case InferenceEnum.WMH:
            output_files.append(os.path.join(base_output_path, "Pred_WMH.nii.gz"))
            output_files.append(os.path.join(base_output_path, "Pred_WMH_synthseg.nii.gz"))
            output_files.append(os.path.join(base_output_path, "Pred_WMH.json"))
...
def build_input_post_process(input_paths, model_name) -> list[pathlib.Path | str]:
        ...
        case InferenceEnum.WMH:
            for mapping_series in MODEL_MAPPING_SERIES_DICT[InferenceEnum.WMH]:
                ...
                if result[0].shape[0] == 1:
                    input_paths.append(
                        input_paths[0].replace("T2FLAIR_AXI.nii.gz", "synthseg_T2FLAIR_AXI_original_synthseg5.nii.gz")
                    )
                    input_paths.append(
                        input_paths[0].replace("T2FLAIR_AXI.nii.gz", "synthseg_T2FLAIR_AXI_original_WMH_PVS.nii.gz")
                    )
        case _:
            pass
    return input_paths
...
def build_analysis(study_path: pathlib.Path):
    ...
    for task_dict in model_dict_values:
        ...
        for model_name, input_paths in task_dict.items():
            ...
            task_output_files = generate_output_files(input_paths, model_name, str(study_path))
            task = Task(
...
) -> InferenceCmd | None:  # -> Optional[List[Tuple[str,str]]]:
